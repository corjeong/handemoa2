						...
						<c:forEach var="comment" items="${commentList}" varStatus="i">
							<div class="list_body">
								<div class="list_row">
									...
									<div class="report">${comment.commentcontent }</div>
									<c:if test="${comment.divisioncode == 1}">
										<div class="report"><a href="/community?postnum=${comment.postnum }" target="_blank">${comment.postnum }</a></div>
									</c:if>
									<c:if test="${comment.divisioncode == 2}">
										<div class="report"><a href="/rankingpost?postnum=${comment.postnum }" target="_blank">${comment.postnum }</a></div>
									</c:if>
								</div>								
							</div>
						</c:forEach>
